...
// הוראה לא לשמור את העמוד בזיכרון (Cache) אלא לטעון מחדש תמיד
export const revalidate = 0;

// שימוש באותם מפתחות שעובדים לנו ב-API של הטלפון!
const supabase = createClient(process.env.SUPABASE_URL!, process.env.SUPABASE_ANON_KEY!);
const CAMPAIGN_ID = '11111111-1111-1111-1111-111111111111';

export default async function SettingsPage() {
  // 1. משיכת השאלות מהמסד (קורה בשרת, ללא CORS!)
  ...
  async function addStep(formData: FormData) {
    'use server'; // מכריח את הקוד לרוץ רק בשרת
    ...
    // רענון העמוד אוטומטית אחרי השמירה
    revalidatePath('/settings');
  }

  // 3. פונקציית שרת למחיקת שאלה
      ...
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <div className="bg-white p-6 rounded-xl shadow-md border border-gray-100">
          ...
          <form action={addStep} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">מזהה נתון (למשל: age, id_number)</label>
                <input name="dataKey" required type="text" dir="ltr"
                  className="w-full border p-2 rounded focus:ring-2 focus:ring-blue-500" placeholder="e.g. id_number" />
              </div>
        ...
        <div className="bg-white p-6 rounded-xl shadow-md border border-gray-100">
          ...
          <div className="space-y-3">
            ...
            {(!steps || steps.length === 0) && <p className="text-gray-500">לא הוגדרו שאלות. המערכת תשתמש בשאלות ברירת המחדל.</p>}
          </div>
        </div>
